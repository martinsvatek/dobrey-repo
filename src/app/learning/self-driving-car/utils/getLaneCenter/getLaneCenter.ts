import { ROAD_CANVAS_WIDTH, ROAD_LANE_COUNT, ROAD_LANE_LEFT_POSITION, ROAD_LANE_WIDTH } from '../../page.consts';
...
export const getLaneCenter = (laneIndex: number): number => {
   ...
   */
  const lanesWidth = ROAD_CANVAS_WIDTH - roadSideWidth * 2 - ROAD_LANE_WIDTH * (ROAD_LANE_COUNT - 1);
  ...
  const laneCenterPositionX =
    roadSideWidth + laneWidth / 2 + Math.min(laneIndex, ROAD_LANE_COUNT - 1) * (laneWidth + ROAD_LANE_WIDTH);
