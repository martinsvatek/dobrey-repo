...
const ShortestPath: FC = () => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      const context = canvas.getContext("2d");
      context &&
        drawShortestPath(context, visitedGridInOrder, shortestPathInOrder);
    }
   ...
   */
  const onMouseInteractionHandler = (
      ...
      while (unvisitedNodes.length > 0) {
        ...
        if (closestNode) {
          ...
          if (row > 5) {
            neighborNodes.push(
              grid[
                (GRID_CANVAS_WIDTH / 10) * (Math.floor(row / 10) - 1) +
                  Math.floor(column / 10)
              ]
            );
          }
          ...
          if (column > 5) {
            neighborNodes.push(
              grid[
                (GRID_CANVAS_WIDTH / 10) * Math.floor(row / 10) +
                  Math.floor(column / 10) -
                  1
              ]
            );
